...
def kfold_ppl(nb_folds, data_size, ppls, people_id, people_count, random_state=0):
...
def train_nnet(X_most, X_val_most, X_test_most, X_group, X_val_group, X_test_group, n_groups, y_tr, y_val):
...
def run():
    ...
    nb_folds = 6
    train_indices, valid_indices = kfold_ppl(nb_folds, tr_len, ppls, people_id, people_count)
    ...
    for i in range(nb_folds):
        ...
        preds_test.append(train_nnet(X_most, X_val_most, X_test_most,
            X_group, X_val_group, X_test_group, n_groups, y_tr, y_val))
    pred = np.mean(np.concatenate(preds_test,1), 1)
    submit = pd.DataFrame({ 'activity_id': act_id, 'outcome': pred })
    submit.to_csv(output_path+'eval_nn_ppl_mix.csv', index=False)
